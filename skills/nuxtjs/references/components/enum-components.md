---
name: nuxtjs-enum-components
description: Nuxt.js 固定枚举组件的 options、展示、输入变体和 useStatusRender 协议
---

# 枚举组件规范

本文适用于新增或修改状态、类型、等级、布尔语义等固定代码枚举的展示与输入组件，以及 `layers/core/composables/useStatusRender.ts`。枚举来自后台可维护注册表时必须同时读取 [business-options.md](business-options.md)；组件落点以 [placement.md](placement.md) 为唯一权威。
...
- 组件从 [placement.md](placement.md) 判定的最窄作用域开始；本文目录结构不构成提升到 `src/components` 或 `layers/core` 的依据。
...
```ts
import type { StatusOption } from '@@/layers/core/composables/useStatusRender'
...
所有枚举展示组件必须复用 `layers/core/composables/useStatusRender.ts`，禁止在各组件重复计算 tag/link 样式。项目缺少该文件时，首次增加枚举展示组件必须按以下契约补齐一次，后续只允许复用或扩展统一契约。
...
`useStatusRender(options, value, mode)` 必须返回 `option`、`tagType`、`tagStyle`、`tagColor`、`tagEffect`、`linkType`、`linkStyle`、`label` 和 `loading` 等 computed 数据。
...
import { useStatusRender, type ModeType } from '@@/layers/core/composables/useStatusRender'
import { options, type Value } from './options'
...
当前枚举已经有 icon 等第四种展示形态的真实调用方时，允许在当前枚举组件内扩展 mode；禁止修改所有枚举共享的 `ModeType`。特殊 mode 必须映射为一个标准 `ModeType` 传给 `useStatusRender`，并在标准 link/tag/text 分支之前渲染。只有当前枚举的 `Option` 可以增加 icon 等业务字段。
...
- 存在展示调用时必须检查 `index.vue` 复用 core `useStatusRender` 且支持 tag/link/text；存在输入调用时必须检查对应变体共用同一 options 和 `v-model:value`。
- 必须检查 `useStatusRender` 没有业务枚举、请求或 mutation，未知值、type 和 effect 的兜底符合统一协议。
- 特殊 mode 必须有真实调用方，且没有污染公共 `ModeType`。
- 必须对新增或修改的 Vue/TypeScript 文件运行项目 format、ESLint 和 typecheck。
